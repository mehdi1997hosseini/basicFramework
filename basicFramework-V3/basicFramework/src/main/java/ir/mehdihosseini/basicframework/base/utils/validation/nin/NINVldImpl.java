package ir.mehdihosseini.basicframework.base.utils.validation.nin;
...
public class NINVldImpl implements ConstraintValidator<NIN, String> {
    ...
    public boolean isValid(String nin, ConstraintValidatorContext context) {
        ...
        return switch (nINType) {
            case SSN_IR -> checkSSN_IR(nin);
            case SSN_COMPANY -> check_SSN_company(nin);
            case PASSPORT_NO -> check_PASSPORT_NO(nin);
            case FIDA -> check_FIDA(nin);
        };
    }
