...
public class DurationAndInstantUtils {
    ...
    public static Duration getDurationOfTimeByTimeUnitType(long expiresIn, TimeUnitType timeUnit) {
        switch (timeUnit) {
            case NANOSECONDS :
                return Duration.ofNanos(expiresIn);
            case MICROSECONDS :
                return Duration.ofNanos(expiresIn * 1000);
            case MILLISECONDS :
            ...
            case HOURS :
            case CUSTOM_HOURS :
                return Duration.ofHours(expiresIn);
            case DAYS :
                return Duration.ofDays(expiresIn);
            default :
    ...
    public static Instant convertToInstant(long value, TimeUnitType unit) {
        switch (unit) {
            ...
            case MICROSECONDS :
                return Instant.ofEpochMilli(value / 1000);
            case MILLISECONDS :
            ...
            case MINUTES :
            case CUSTOM_MINUTES:
                return Instant.ofEpochSecond(value * 60);
            case HOURS :
            case CUSTOM_HOURS:
                return Instant.ofEpochSecond(value * 3600);
            case DAYS :
                return Instant.ofEpochSecond((value * 3600) * 24);
            default :
